...
from alphapose.utils.metrics import (
    DataLogger,
    calc_accuracy,
    calc_integral_accuracy,
    evaluate_mAP,
)
...
def train(opt, train_loader, m, criterion, optimizer, writer):
    ...
    for i, (inps, labels, label_masks, _, bboxes) in enumerate(train_loader):
        ...
        if cfg.LOSS.get("TYPE") == "MSELoss":
            ...
            acc = calc_integral_accuracy(
                output, labels, label_masks, output_3d=False, norm_type=norm_type
            )
        ...
        if opt.board:
            board_writing(
                writer, loss_logger.avg, acc_logger.avg, opt.trainIters, "Train"
            )
...
def validate(m, opt, heatmap_to_coord, batch_size=20):
    ...
    for inps, crop_bboxes, bboxes, img_ids, scores, imghts, imgwds in tqdm(
        ...
        for i in range(output.shape[0]):
            ...
            data["score"] = float(
                scores[i] + np.mean(pose_scores) + np.max(pose_scores)
            )
            data["category_id"] = 1
    ...
    res = evaluate_mAP(
        os.path.join(opt.work_dir, "test_kpt.json"),
        ann_type="keypoints",
...
def validate_gt(m, opt, cfg, heatmap_to_coord, batch_size=20):
    gt_val_dataset = builder.build_dataset(
        cfg.DATASET.VAL, preset_cfg=cfg.DATA_PRESET, train=False
    )
    ...
    for inps, labels, label_masks, img_ids, bboxes in tqdm(
        gt_val_loader, dynamic_ncols=True
    ):
        if isinstance(inps, list):
            inps = [inp.cuda() for inp in inps]
        else:
    ...
    with open(os.path.join(opt.work_dir, "test_gt_kpt.json"), "w") as fid:
        json.dump(kpt_json, fid)
    res = evaluate_mAP(
        os.path.join(opt.work_dir, "test_gt_kpt.json"),
        ann_type="keypoints",
        ann_file=os.path.join(cfg.DATASET.VAL.ROOT, cfg.DATASET.VAL.ANN),
    )
    return res
...
def main():
    ...
    train_dataset = builder.build_dataset(
        cfg.DATASET.TRAIN, preset_cfg=cfg.DATA_PRESET, train=True
    )
    ...
    for i in range(cfg.TRAIN.BEGIN_EPOCH, cfg.TRAIN.END_EPOCH):
        ...
        if opt.board:
            writer.add_scalar("Train/lr", current_lr, opt.epoch)
        ...
        if (i + 1) % opt.snapshot == 0:
            ...
            # torch.save(
            #     m.module.state_dict(),
            #     "./exp/{}-{}/model_{}.pth".format(opt.exp_id, cfg.FILE_NAME, opt.epoch),
            # )
            ...
            with torch.no_grad():
                ...
                if rcnn_AP > best_ap:
                    logger.info(
                        f"##### Stored new best model: {rcnn_AP} > {best_ap}"
                    )
                    best_ap = rcnn_AP
                    torch.save(
                        m.module.state_dict(),
                        "./exp/{}-{}/model_best.pth".format(
                ...
                # Tensorboard
                if opt.board:
                    writer.add_scalar("Val/gt_mAP", gt_AP, opt.epoch)
                    writer.add_scalar("Val/rcnn_mAP", rcnn_AP, opt.epoch)
        ...
        if i == cfg.TRAIN.DPG_MILESTONE:
            ...
            # Reset dataset
            train_dataset = builder.build_dataset(
                cfg.DATASET.TRAIN, preset_cfg=cfg.DATA_PRESET, train=True, dpg=True
            )
            train_loader = torch.utils.data.DataLoader(
                train_dataset,
                batch_size=cfg.TRAIN.BATCH_SIZE * num_gpu,
