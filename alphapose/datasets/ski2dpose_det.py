...
class Ski2dpose_det(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_path = "./data/ski2dpose/val/%d.webp" % img_id
        ...
        return (
            inp,
            torch.Tensor(bbox),
            torch.Tensor([det_res["bbox"]]),
            torch.Tensor([det_res["image_id"]]),
            torch.Tensor([det_res["score"]]),
            torch.Tensor([imght]),
...
@DATASET.register_module
class Ski2dposeWithCoG_det(Ski2dpose_det):
    """ COCO human detection box dataset.
